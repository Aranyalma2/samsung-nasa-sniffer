...
function applyFilters() {
	...
	filteredPackets = packets.filter((packet) => {
		// Source filter
		if (sourceFilter && !packet.source.toLowerCase().includes(sourceFilter) && !packet.sourceReadable.toLowerCase().includes(sourceFilter)) {
			return false;
		}

		// Destination filter
		if (destFilter && !packet.destination.toLowerCase().includes(destFilter) && !packet.destinationReadable.toLowerCase().includes(destFilter)) {
			return false;
		}
...
function updateUI() {
	...
	packetListEl.innerHTML = packetsToShow.map((packet, index) => createPacketElement(packet, index, packet.timestamp === selectedPacketTimestamp)).join("");
...
// Event listeners
filterSource.addEventListener("input", () => {
	currentPage = 1;
	applyFilters();
});
...
filterDataType.addEventListener("change", () => {
	currentPage = 1;
	applyFilters();
});
filterMessageName.addEventListener("input", () => {
	currentPage = 1;
	applyFilters();
});
filterMessageValue.addEventListener("input", () => {
	currentPage = 1;
	applyFilters();
});
filterRawValue.addEventListener("input", () => {
	currentPage = 1;
	applyFilters();
});
...
firstPageBottomBtn.addEventListener("click", () => {
	currentPage = 1;
	updateUI();
});
